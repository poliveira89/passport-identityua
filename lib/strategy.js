...
Strategy.prototype.userProfile = function(token, tokenSecret, params, done) {
  ...
  if (hasScope) {
    const strategy = this;
    const getScopeData = param =>
      new Promise((resolve, reject) => {
        strategy._oauth.get(
          this._userProfileURL + "?format=json&scope=" + param,
          token,
          tokenSecret,
          function(err, body, res) {
            ...
            try {
              ...
              let _err = new Error("Failed to parse user profile");
              console.error(_err);
              reject(err);
            }
          }
    ...
    const getAllScopesData = scope.map(param => getScopeData(param));

    const data = Promise.all(getAllScopesData)
      .then(results => {
